%% STUDY MEMRISTIVE DEEP BELIEF NET
...
file_name=cat(2,sprintf('DBN_%depoch_%dmb_%dCD_%dbits_',n_epochs,mini_batch_size,nb_CD,nb_bits),scheme);
mkdir(file_name)
current_dir=pwd;
...
for ind_beta=1:length(beta_tab)
    ...
    parfor ind_dt=1:length(r_tab)
        ...
        [model,momentum,param]=init('model',{'gen','flip'},n_epochs,n_layer,lr,...
            ...
            [1,2,3,4],[dt_max/r_tab(ind_dt),dt_max/r_tab(ind_dt),dt_max/r_tab(ind_dt),dt_max/r_tab(ind_dt)],...
...
for ind_var=1:length(var_tab)
    fprintf(cat(2,'\n',file_name,sprintf(' : intra =%.2g',var_tab(ind_var))));
    parfor ind_dt=1:length(r_tab)
        data.train=data_set.train.inputs;
        data.test=data_set.test.inputs;
        [model,momentum,param]=init('model',{'gen','flip'},n_epochs,n_layer,lr,...
...
for ind_var=1:length(var_tab)
    fprintf(cat(2,'\n',file_name,sprintf(' : inter =%.2g',var_tab(ind_var))));
    parfor ind_dt=1:length(r_tab)
        data.train=data_set.train.inputs;
        data.test=data_set.test.inputs;
        [model,momentum,param]=init('model',{'gen','flip'},n_epochs,n_layer,lr,...
